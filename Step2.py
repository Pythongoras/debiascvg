...
def find_v_mnt(beta, n):
    ...
    @return: List[np.array, int]: [v, #constant pieces of v]
    ...
    for i, v in enumerate(proj_list):
        ...
        if cur_value < opt_value:
            opt_value = cur_value
            opt_l = i
    return [proj_list[opt_l], opt_l + 1]
...
# LASSO
#######
def find_v_lasso(beta, l1_bound, n):
    """
    Find the optimal v in step 2 for LASSO.
    
    @param beta np.array:
    @param l1_bound float:
    @param n float:
    @return np.array
    """
    ...
    for i in range(1, sb+1):
        vs = find_sparse_proj(beta, l1_bound, i)
...
#######
def find_v_slope(beta, n, C, su):
    """
    Find the optimal v in step 2 for SLOPE.
    
    @param beta np.array(p, ): the beta obtained by the first step.
		@param n: the sample size.
    @param C float: the constant C in upper bound.
    @param su integer: the upper bound of number of non-sparse coordinates
    @return np.array
    """
    p = len(beta)
    ...
    non_zero_mass = np.clip(C**2 * su * log(2*p*exp(1)/su) / n - np.sum(beta[zero_idx]**2), su, None)
